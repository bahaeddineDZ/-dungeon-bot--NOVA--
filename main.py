...
async def choose_or_upgrade_role(ctx):
    ...
    await ctx.send(embed=main_embed, view=ExistingSpecView())

# ----------------------------------------------------------------------- فاصل --- أمر النهب -----------
from datetime import datetime, timedelta
import time
# --- دوال مساعدة من roles_utils.py ---
def get_role_level_bonus(role, rank_name):
    ranks = ["نبيل", "شجاع", "فارسي", "أسطوري"]
    if rank_name not in ranks:
        return None
    ...
    if role == "محارب":
        ...
        return {"type": "reflect_steal", "percentage": [20, 40, 60, 80][index]}
    else:
        return None
...
# --- أمر درع لتفعيل درع سورا ---
@bot.command(name="درع")
async def activate_sora_shield_command(ctx):
    user_id = str(ctx.author.id)
    init_user(user_id, ctx.author.display_name)
    data = load_data()
    user = data[user_id]
    ...
    if user.get("specialization", {}).get("type", "") == "سورا":
        ...
        save_data(data)
        await ctx.send("✅ تم تفعيل درع سورا لمدة ساعة واحدة.")
    else:
        await ctx.send("❌ فقط سورا يمكنه تفعيل الدرع.")
        return
    # ⛨ تفعيل الدرع
    protection_until = datetime.utcnow() + timedelta(minutes=60)
    user["sora_shield_until"] = protection_until.isoformat()
...
# --- أمر نهب مع دمج خاصية درع سورا وعكس السرقة ---
@bot.command(name="نهب")
async def steal(ctx, target: discord.Member):
    # فحص التبريد
    can_steal, cooldown_msg = check_cooldown(ctx.author.id, "نهب")
    if not can_steal:
        await ctx.send(f"⏳ يجب الانتظار قبل المحاولة مرة أخرى: {cooldown_msg}")
        return
    ...
    if target_id == user_id:
        await ctx.send("❌ لا يمكنك نهب نفسك.")
        return
    ...
    if now_ts < protection_end:
        ...
        await ctx.send(f"🛡️ {target.name} تحت حماية الشامان لمدة {minutes} دقيقة و{seconds} ثانية أخرى.")
        return

    victim_balance = victim["balance"]["دولار"]
    if victim_balance < 50:
        await ctx.send(f"❌ {target.name} لا يملك ما يكفي ليتم نهبه.")
        return
    ...
    if shield_active:
        ...
        save_data(data)
        update_cooldown(ctx.author.id, "نهب")
        await ctx.send(f"❌ درع سورا مفعل! تم عكس {amount_reflected}$ من محاولتك وتمت إضافتها إلى {target.name}.")
        return
    ...
    if is_sora:
        ...
        if reversed_success:
            save_data(data)
            update_cooldown(ctx.author.id, "نهب")
            await ctx.send(f"❌ فشلت محاولتك! تم عكس السرقة ⚡ وتم خصم المبلغ منك وتحويله إلى {target.name}.")
            return
    ...
    update_cooldown(ctx.author.id, "نهب")
    save_data(data)
    await ctx.send(f"💰 لقد نهبت {target.name} وسرقت {steal_amount}$ بنجاح!")
...
async def protect(ctx, member: discord.Member = None):
    ...
    if user.get("role") != "شامان":
        await ctx.send("🛡️ فقط الشامان يستطيع استخدام أمر الحماية.")
        return
    ...
    if rank == "أسطوري" and member:
        ...
        await ctx.send("❌ لا يمكنك حماية لاعب آخر إلا إذا كنت في رتبة أسطوري.")
        return
    else:
        target_id = user_id
    ...
    if not target:
        await ctx.send("🚫 اللاعب غير موجود في البيانات.")
        return

    if target.get("shield_until", 0) > now:
        await ctx.send(f"🛡️ {member.mention if member else 'أنت'} محمي بالفعل.")
        return
    now = time.time()
    ...
    if target_id == user_id:
        await ctx.send(f"🛡️ تم تفعيل الحماية لك لمدة `{duration // 60}` دقيقة.")
    else:
        await ctx.send(f"🛡️ تم تفعيل الحماية لـ {member.mention} لمدة `{duration // 60}` دقيقة.")
    user["shield_until"] = time.time() + duration
    save_data(data)  # تأكد أنها موجودة فورًا بعد التحديث

    save_data(data)

# ------------------------------------------------------------------------------ أمر الانتقام -----------
@bot.command(name="انتقام")
async def revenge(ctx, target: discord.Member = None):
    if target is None:
        await ctx.send("❗ الرجاء تحديد الشخص الذي تريد الانتقام منه.\nمثال: `انتقام @اسم_العضو`")
        return
    ...
    if target_id == user_id:
        await ctx.send("❌ لا يمكنك الانتقام من نفسك.")
        return
    ...
    if role != "محارب":
        await ctx.send("❌ فقط المحارب يمكنه الانتقام.")
        return
    ...
    if not revenge_entry:
        await ctx.send(f"⚔️ لا يوجد سجل نهب من {target.name} لتنتقم منه.")
        return
    ...
    if time.time() - revenge_entry["time"] > 86400:
        ...
        await ctx.send(f"⌛ انتهت صلاحية الانتقام من {target.name}.")
        save_data(data)
        return
    ...
    if revenge_amount <= 0:
        await ctx.send(f"💸 {target.name} لا يملك ما يمكن انتزاعه.")
        return
...
async def buy(ctx):
    allowed, remaining = check_cooldown(ctx.author.id, "شراء")
    if not allowed:
        await ctx.send(
            f"⏳ الرجاء الانتظظار {remaining} ثانية قبل استخدام هذا الأمر مرة أخرى."
        )
        return
    ...
    class ItemSelect(Select):
        def __init__(self, prices):
            ...
            super().__init__(
                placeholder="اختر العنصر المراد شراؤه",
                options=options,
                min_values=1,
                max_values=1
            )
        ...
        async def callback(self, interaction: Interaction):
            ...
            price_per_unit = self.prices.get(
                selected_item,
                next(i for i in store_items if i["name"] == selected_item)["price"]
            )
            
            # حساب الكميات الممكنة
            max_affordable = user_balance // price_per_unit
            
            if max_affordable == 0:
                await interaction.response.send_message(
                    f"❌ لا يمكنك شراء {selected_item} - رصيدك غير كافٍ!\n💰 تحتاج: {price_per_unit:,}$ | لديك: {user_balance:,}$",
                    ephemeral=True
                )
                return
            ...
            class BuyQuantityView(View):
                ...
                async def buy_one(self, btn_interaction: Interaction, button: Button):
                    await self.set_quantity(btn_interaction, 1)
                
                @discord.ui.button(label="🔟 عشرة", style=ButtonStyle.secondary)
                async def buy_ten(self, btn_interaction: Interaction, button: Button):
                    qty = min(10, max_affordable)
                    await self.set_quantity(btn_interaction, qty)
                
                @discord.ui.button(label="💯 مائة", style=ButtonStyle.primary)
                async def buy_hundred(self, btn_interaction: Interaction, button: Button):
                    qty = min(100, max_affordable)
                    await self.set_quantity(btn_interaction, qty)
                
                @discord.ui.button(label="🔄 نصف ما أستطيع", style=ButtonStyle.primary)
                async def buy_half_max(self, btn_interaction: Interaction, button: Button):
                    qty = max(1, max_affordable // 2)
                    await self.set_quantity(btn_interaction, qty)
                
                @discord.ui.button(label="💸 كل ما أستطيع", style=ButtonStyle.danger)
                async def buy_max(self, btn_interaction: Interaction, button: Button):
                    await self.set_quantity(btn_interaction, max_affordable)
                
                @discord.ui.button(label="✍️ كتابة يدوية", style=ButtonStyle.success, row=1)
                async def manual_input(self, btn_interaction: Interaction, button: Button):
                    class ManualQuantityModal(Modal, title="كتابة الكمية يدوياً"):
                        def __init__(self):
                            super().__init__()
                            self.qty_input = TextInput(
                                label=f"أدخل الكمية المطلوبة (الحد الأقصى: {max_affordable:,})",
                                placeholder=f"مثال: {min(5, max_affordable)}",
                                required=True,
                                max_length=10
                            )
                        ...
                        async def on_submit(self, modal_interaction: Interaction):
                            try:
                                ...
                                if qty <= 0:
                                    await modal_interaction.response.send_message("❌ الكمية يجب أن تكون أكبر من صفر!", ephemeral=True)
                                    return
                                if qty > max_affordable:
                                    await modal_interaction.response.send_message(
                                        f"❌ لا يمكنك شراء {qty:,} قطعة!\n💰 الحد الأقصى الذي تستطيع شراؤه: {max_affordable:,}",
                                        ephemeral=True
                                    )
                                    return
                                
                                await self.parent.confirm_purchase(modal_interaction, qty)
                                
                            except ValueError:
                                await modal_interaction.response.send_message("❌ أدخل رقماً صحيحاً فقط!", ephemeral=True)

                    await btn_interaction.response.send_modal(ManualQuantityModal())
                
                async def set_quantity(self, btn_interaction: Interaction, quantity: int):
                    self.selected_quantity = quantity
                    await self.confirm_purchase(btn_interaction, quantity)
                
                async def confirm_purchase(self, btn_interaction: Interaction, quantity: int):
                    total_cost = price_per_unit * quantity
                    remaining_balance = user_balance - total_cost
                    ...
                    embed.add_field(
                        name="🧮 العملية الحسابية",
                        value=f"{quantity:,} × {price_per_unit:,}$ = **{total_cost:,}$**",
                        inline=False
                    )
                    ...
                    embed.add_field(
                        name="💸 الرصيد بعد الشراء",
                        value=f"{remaining_balance:,}$",
                        inline=True
                    )
                    ...
                    class FinalConfirmView(View):
                        ...
                        async def confirm(self, final_interaction: Interaction, button: Button):
                            # التأكد من الرصيد مرة أخرى
                            current_data = load_data()
                            current_balance = current_data[user_id]["balance"]["دولار"]
                            ...
                            # تنفيذ عملية الشراء
                            current_data[user_id]["balance"]["دولار"] -= total_cost
                            current_data[user_id].setdefault("حقيبة", []).extend([selected_item] * quantity)
                            ...
                            success_embed = Embed(
                                title="✅ تمت عملية الشراء بنجاح!",
                                description=f"🎉 تم شراء **{quantity:,}** من {selected_item}",
                                color=0x00ff00
                            )
                            ...
                            success_embed.add_field(
                                name="💳 رصيدك الجديد",
                                value=f"{current_data[user_id]['balance']['دولار']:,}$",
                                inline=True
                            )
                            ...
                            await final_interaction.response.edit_message(
                                embed=success_embed,
                                view=None
                            )
                        
                        @discord.ui.button(label="❌ إلغاء", style=ButtonStyle.danger)
                        async def cancel(self, final_interaction: Interaction, button: Button):
                            await final_interaction.response.edit_message(
            ...
            embed = Embed(
                ...
                description=f"**العنصر:** {selected_item}\n**السعر:** {price_per_unit:,}$ للقطعة الواحدة\n**رصيدك:** {user_balance:,}$\n**الحد الأقصى:** {max_affordable:,} قطعة",
                color=0x3498db
            )
            
            await interaction.response.send_message(
                embed=embed,
                view=BuyQuantityView(),
                ephemeral=True
            )
    ...
    class PurchaseView(View):
        def __init__(self, prices):
            super().__init__(timeout=60)
            self.add_item(ItemSelect(prices))

    embed = Embed(
        title="🛍️ متجر الشراء",
        description=f"💰 رصيدك: **{user_balance:,}$**\nاختر العنصر الذي تريد شراءه:",
        color=0x2ecc71
    )
    
    await ctx.send(embed=embed, view=PurchaseView(prices))
    update_cooldown(user_id, "شراء")
# ---------------------------------------------------- بيع -----------------فاصل-----------
...
async def sell(ctx):
    ...
    if not allowed:
        await ctx.send(f"⏳ الرجاء الانتظار {remaining} قبل استخدام هذا الأمر مجددًا.")
        return

    init_user(user_id, ctx.author.display_name)
    data = load_data()
    user_data = data.get(user_id, {})
    حقيبة = user_data.get("حقيبة", [])

    if not حقيبة:
        await ctx.send("🎒 لا توجد عناصر في حقيبتك.")
        return

    # عناصر قابلة للبيع فقط
    sellable_counts = {}
    for item in حقيبة:
        # التحقق من وجود العنصر في الأسعار
        prices = update_prices_if_needed()
        if item in prices or any(store_item["name"] == item for store_item in store_items):
            sellable_counts[item] = sellable_counts.get(item, 0) + 1

    if not sellable_counts:
        await ctx.send("⚠️ لا توجد عناصر قابلة للبيع في حقيبتك.")
        return

    class ItemSellSelect(Select):
        def __init__(self):
            super().__init__(placeholder="اختر العنصر المراد بيعه", min_values=1, max_values=1)
            current_prices = update_prices_if_needed()
            
            options = []
            for item, count in sellable_counts.items():
                price = current_prices.get(item)
                if not price:
                    # البحث في المتجر عن السعر الأساسي
                ...
                if price:
                    options.append(
                        discord.SelectOption(
                            label=item[2:].strip() if len(item) > 2 else item,
                            description=f"السعر: {price:,}$ | لديك: {count}",
            ...
            if options:
                self.options = options[:25]  # حد أقصى 25 خيار
        ...
        async def callback(self, interaction: Interaction):
            ...
            selected_item = self.values[0]
            max_quantity = sellable_counts[selected_item]
            
            current_prices = update_prices_if_needed()
            sale_price = current_prices.get(selected_item)
            if not sale_price:
                for store_item in store_items:
                    if store_item["name"] == selected_item:
                        sale_price = store_item["price"]
                        break
            
            if not sale_price:
                await interaction.response.send_message("❌ لا يمكن تحديد سعر هذا العنصر.", ephemeral=True)
                return
            ...
            class SellQuantityView(View):
                ...
                @discord.ui.button(label="1️⃣ واحد", style=ButtonStyle.secondary)
                async def sell_one(self, btn_interaction: Interaction, button: Button):
                    await self.confirm_sale(btn_interaction, 1)
                
                @discord.ui.button(label="🔟 عشرة", style=ButtonStyle.secondary)
                async def sell_ten(self, btn_interaction: Interaction, button: Button):
                    qty = min(10, max_quantity)
                    await self.confirm_sale(btn_interaction, qty)
                
                @discord.ui.button(label="💯 مائة", style=ButtonStyle.primary)
                async def sell_hundred(self, btn_interaction: Interaction, button: Button):
                    qty = min(100, max_quantity)
                    await self.confirm_sale(btn_interaction, qty)
                
                @discord.ui.button(label="🔄 نصف ما أملك", style=ButtonStyle.primary)
                async def sell_half(self, btn_interaction: Interaction, button: Button):
                    qty = max(1, max_quantity // 2)
                    await self.confirm_sale(btn_interaction, qty)
                
                @discord.ui.button(label="💸 بيع الكل", style=ButtonStyle.danger)
                async def sell_all(self, btn_interaction: Interaction, button: Button):
                    await self.confirm_sale(btn_interaction, max_quantity)
                
                @discord.ui.button(label="✍️ كتابة يدوية", style=ButtonStyle.success, row=1)
                async def manual_sell(self, btn_interaction: Interaction, button: Button):
                    class ManualSellModal(Modal, title="كتابة الكمية يدوياً"):
                        def __init__(self):
                            super().__init__()
                            self.qty_input = TextInput(
                                label=f"أدخل الكمية المراد بيعها (الحد الأقصى: {max_quantity})",
                                placeholder=f"مثال: {min(5, max_quantity)}",
                                required=True,
                                max_length=10
                            )
                            self.add_item(self.qty_input)
                        ...
                        async def on_submit(self, modal_interaction: Interaction):
                            try:
                                ...
                                if qty > max_quantity:
                                    await modal_interaction.response.send_message(
                                        f"❌ لا تملك {qty:,} قطعة!\n📦 الحد الأقصى الذي تملكه: {max_quantity}",
                                        ephemeral=True
                                    )
                                    return
                                
                                await self.parent.confirm_sale(modal_interaction, qty)
                                
                            except ValueError:
                                await modal_interaction.response.send_message("❌ أدخل رقماً صحيحاً فقط!", ephemeral=True)

                    await btn_interaction.response.send_modal(ManualSellModal())
                
                async def confirm_sale(self, btn_interaction: Interaction, quantity: int):
                    total_earning = sale_price * quantity
                    current_balance = data[user_id]["balance"]["دولار"]
                    new_balance = current_balance + total_earning
                    ...
                    embed.add_field(
                        name="🧮 العملية الحسابية",
                        value=f"{quantity:,} × {sale_price:,}$ = **{total_earning:,}$**",
                        inline=False
                    )
                    ...
                    embed.add_field(
                        name="📊 الربح المحقق",
                        value=f"+{total_earning:,}$",
                        inline=True
                    )
                    
                    class FinalSellConfirmView(View):
                        def __init__(self):
                            super().__init__(timeout=30)
                        ...
                        async def confirm(self, final_interaction: Interaction, button: Button):
                            # التأكد من وجود العناصر
                            current_data = load_data()
                            current_bag = current_data[user_id].get("حقيبة", [])
                            available_count = current_bag.count(selected_item)
                            
                            if available_count < quantity:
                                await final_interaction.response.send_message(
                                    f"❌ لم تعد تملك {quantity} من {selected_item}!\nلديك فقط {available_count}",
                                    ephemeral=True
                                )
                                return
                            ...
                            # تنفيذ عملية البيع
                            for _ in range(quantity):
                                current_bag.remove(selected_item)
                            
                            current_data[user_id]["balance"]["دولار"] += total_earning
                            current_data[user_id]["حقيبة"] = current_bag
                            save_data(current_data)
                            
                            success_embed = Embed(
                                title="✅ تمت عملية البيع بنجاح!",
                                description=f"🎉 تم بيع **{quantity:,}** من {selected_item}",
                                color=0x00ff00
                            )
                            ...
                            success_embed.add_field(
                                name="📦 المتبقي",
                                value=f"{current_bag.count(selected_item)} قطعة" if selected_item in current_bag else "لا يوجد",
                                inline=True
                            )
                    ...
                    await btn_interaction.response.edit_message(
                        embed=embed,
                        view=FinalSellConfirmView()
                    )

            embed = Embed(
                title="💰 اختيار كمية البيع",
                description=f"**العنصر:** {selected_item}\n**السعر:** {sale_price:,}$ للقطعة الواحدة\n**لديك:** {max_quantity} قطعة\n**القيمة الإجمالية:** {sale_price * max_quantity:,}$",
                color=0xe67e22
            )
            
            await interaction.response.send_message(
                embed=embed,
                view=SellQuantityView(),
                ephemeral=True
            )

    class SellMainView(View):
        def __init__(self):
            super().__init__(timeout=60)
            self.add_item(ItemSellSelect())

    total_value = sum(
        sellable_counts[item] * (update_prices_if_needed().get(item) or next((si["price"] for si in store_items if si["name"] == item), 0))
        for item in sellable_counts
    )

    embed = Embed(
        title="💰 متجر البيع",
        description=f"📦 لديك {len(sellable_counts)} نوع من العناصر\n💎 القيمة الإجمالية: **{total_value:,}$**\nاختر العنصر الذي تريد بيعه:",
        color=0xe67e22
    )
    
    await ctx.send(embed=embed, view=SellMainView())
...
# ----------------------------------------------- تبريد العمل -------------# ----------------------------------------------- تبريد العمل ------------- فاصل-------------
# حساب ثروة اللاعب
# حساب ثروة اللاعب
# حساب الثروة الكلية للمستخدم
...
@bot.command(name="صياد")
async def fisher_shop(ctx):
    user_id = str(ctx.author.id)
    init_user(user_id, ctx.author.display_name)
    data = load_data()
...
async def show_commands(ctx, return_from_sub=False):
...
@bot.command(name="مزارع")
async def farm_shop(ctx):
    user_id = str(ctx.author.id)
    init_user(user_id, ctx.author.display_name)
    data = load_data()
...
@bot.command(name="زرع")
async def plant_seed(ctx):
    user_id = str(ctx.author.id)
    init_user(user_id, ctx.author.display_name)
    data = load_data()
    user_data = data[user_id]
...
@bot.command(name="خبرة")
async def experience_leaderboard(ctx):
...
@bot.command(name="تحدي_سريع")
async def quick_challenge_menu(ctx):
...
async def challenge_player(ctx, opponent: discord.Member, currency: str, amount: int):
...
async def quick_duel(ctx, opponent: discord.Member = None):
...
@bot.command(name="سباق_سريع")
async def quick_racing_menu(ctx):
...
@bot.command(name="سباق")
async def racing_game(ctx, opponent: discord.Member, currency: str, amount: int):
...
@bot.command(name="أنشطتي")
async def my_activities(ctx):
...
@bot.command(name="مسؤول")
async def admin_give(ctx, amount: int, currency: str):
...
@bot.command(name="تبريد_سراديب")
async def dungeon_cooldowns(ctx):
